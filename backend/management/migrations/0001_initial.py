    ...
    operations = [
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('http_timeout', models.IntegerField(default=10, help_text='The maximum amount of time (in seconds) that the Capybara application will wait for a response to an HTTP request before closing the connection. This setting helps to avoid indefinitely hanging requests.', verbose_name='HTTP session timeout')),
                ('http_default_headers', models.JSONField(default=[{'Key': 'Content-Type', 'Value': 'application/json'}], help_text='Default headers to include in HTTP requests. This can include headers like "Content-Type", "Authorization", and other custom headers required by the API endpoints the application interacts with.', validators=[wanderswiss.base.validators.json_validators.json_list_dict_validator], verbose_name='HTTP default headers')),
                ('http_max_workers', models.IntegerField(default=10, help_text='Xxx.', verbose_name='HTTP Max workers')),
                ('ssh_timeout', models.IntegerField(default=10, help_text='The maximum amount of time (in seconds) that the Capybara application will wait for a response to an SSH request before closing the connection. This helps to prevent indefinitely hanging SSH sessions.', verbose_name='SSH session timeout')),
                ('ssh_repeat', models.IntegerField(default=2, help_text='The number of times the application will retry an SSH connection if the initial attempt fails. This setting can help improve reliability in unstable network environments.', verbose_name='SSH repeat connection')),
                ('ssh_invalid_responses', models.JSONField(default=['% Invalid input detected', 'syntax error, expecting', 'Error: Unrecognized command', '%Error', 'command not found', 'Syntax Error: unexpected argument', '% Unrecognized command found at', 'invalid input detected', 'cdp is not enabled', 'incomplete command', 'no spanning tree instance exists', 'lldp is not enabled', 'snmp agent not enabled'], help_text='A list of strings that represent invalid responses from the host. These are used to identify and handle errors during SSH sessions. For example, certain network devices might return specific error messages like "invalid input detected" or "command not found" when a command fails.', verbose_name='SSH invalid responses')),
                ('ssh_max_workers', models.IntegerField(default=10, help_text='Xxx.', verbose_name='SSH Max workers')),
